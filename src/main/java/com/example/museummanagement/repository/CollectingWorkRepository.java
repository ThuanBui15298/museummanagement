...
public interface CollectingWorkRepository extends JpaRepository<CollectingWork, Long> {
    ...
    @Query(value = "select * from news n where n.status = 1 and n.type = 6 and lower(concat(coalesce(n.name,''), coalesce(n.title ,''))) like lower(:search)", nativeQuery = true)
    Page<CollectingWork> findAllBySearch(Pageable pageable, @Param("search") String search);
}
